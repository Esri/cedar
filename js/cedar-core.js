...
var Cedar = function Cedar(options){
  ...
  //hold onto the options
  this.options = options;

  // Array to hold event handlers
  this._events = [];

  // Default params for the query
  if(options.query){
 ...
 * Getter/Setters
 */
Cedar.prototype.mappings = function(/* object or null */){
  //setter
  if (arguments.length > 0){
    if(typeof arguments[0] === "object"){
      this._mappings = arguments[0] 
    }else{
      throw 'Cedar.mappings() requires an object';
    }
...
Cedar.prototype.query = function(/* object or null */){
  //setter
  if (arguments.length > 0){
    if(typeof arguments[0] === "object"){
...
Cedar._generateServiceQueryUrl = function(serviceUrl, params) {
  ...
  if(params.group) {
    this._query.groupByFieldsForStatistics = params.group;
  }
  
  if(params.count) {
    this._query.orderByFields = params.count + "_SUM";
    this._query.outStatistics = JSON.stringify([{"statisticType": "sum", "onStatisticField": params.count, "outStatisticFieldName": params.count + "_SUM"}]);
  }

  var dataUrl = serviceUrl + "/query?" + this._serializeQueryParams(this._query);
